<template>
  <div class="rx-users">
    ...
    <section class="rx-toolbar">
      <div class="rx-search">
        ...
        <input type="text" class="rx-search__input" placeholder="Buscar por nombre, email o teléfono…" v-model="filters.search" @input="debouncedSearch" />
    ...
    <section class="rx-datagrid">
      <div v-if="loading" class="rx-state"><div class="rx-spinner"></div><p>Cargando usuarios…</p></div>
      <div v-else-if="error" class="rx-state"><font-awesome-icon :icon="['fas', 'exclamation-triangle']" class="rx-state__icon" /><h3>Error</h3><p>{{ error }}</p><button class="rx-btn rx-btn--accent" @click="loadUsers">Reintentar</button></div>
      <div v-else-if="users.length === 0" class="rx-state"><font-awesome-icon :icon="['fas', 'users']" class="rx-state__icon" /><h3>Sin usuarios</h3><p>{{ hasActiveFilters ? 'Ajusta los filtros' : 'No hay usuarios registrados' }}</p></div>
      <div v-else class="rx-table-wrap">
        <table class="rx-table">
          <thead>
            <tr>
              <th @click="sortBy('name')" class="rx-th--sortable">Usuario <font-awesome-icon :icon="['fas', getSortIconFA('name')]" class="rx-sort" :class="{active: filters.sortBy==='name'}" /></th>
              <th @click="sortBy('role')" class="rx-th--sortable">Rol <font-awesome-icon :icon="['fas', getSortIconFA('role')]" class="rx-sort" :class="{active: filters.sortBy==='role'}" /></th>
              <th @click="sortBy('status')" class="rx-th--sortable">Estado <font-awesome-icon :icon="['fas', getSortIconFA('status')]" class="rx-sort" :class="{active: filters.sortBy==='status'}" /></th>
              <th>Verificación</th>
              <th @click="sortBy('created_at')" class="rx-th--sortable">Registrado <font-awesome-icon :icon="['fas', getSortIconFA('created_at')]" class="rx-sort" :class="{active: filters.sortBy==='created_at'}" /></th>
              <th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="user in users" :key="user.id" class="rx-row">
              <td>
                <div class="rx-user-cell">
                  <div class="rx-avatar-wrap">
                    <img v-if="user.photo" :src="user.photo" :alt="user.name" class="rx-avatar-img" @error="(e: Event) => ((e.target as HTMLImageElement).style.display = 'none')" />
              ...
              <td>
                <span class="rx-badge" :style="{ color: getRoleConfig(user.role).color, background: getRoleConfig(user.role).bg }">
                  {{ getRoleConfig(user.role).label }}
                </span>
              </td>
              <td>
                <span class="rx-badge" :style="{ color: getStatusConfig(user.status).color, background: getStatusConfig(user.status).bg }">
                  {{ getStatusConfig(user.status).label }}
                </span>
              </td>
              <td>
                <span class="rx-verif" :class="user.verification_status === 'verified' ? 'rx-verif--yes' : 'rx-verif--no'">
                  <font-awesome-icon :icon="['fas', user.verification_status === 'verified' ? 'check-circle' : 'clock']" />
              ...
              <td>
                <div class="rx-actions">
                  <button class="rx-act rx-act--edit" @click="openEditModal(user)" title="Editar"><font-awesome-icon :icon="['fas', 'pen']" /></button>
                  <button class="rx-act" :class="user.status === 'active' ? 'rx-act--reject' : 'rx-act--approve'" @click="toggleUserStatus(user)" :title="user.status === 'active' ? 'Desactivar' : 'Activar'" :disabled="statusUpdateLoading[user.id]">
                    <font-awesome-icon :icon="['fas', statusUpdateLoading[user.id] ? 'spinner' : (user.status === 'active' ? 'lock' : 'lock-open')]" :spin="statusUpdateLoading[user.id]" />
                  </button>
                  <button class="rx-act rx-act--delete" @click="confirmDelete(user)" title="Eliminar"><font-awesome-icon :icon="['fas', 'trash']" /></button>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <div v-if="pagination.lastPage > 1" class="rx-pagination">
          ...
          <div class="rx-pg-pages">
            <button v-for="page in visiblePages" :key="page" class="rx-pg-page" :class="{ active: page === pagination.currentPage }" @click="changePage(page)" :disabled="page === '...' || paginationLoading">{{ page }}</button>
          </div>
          <span class="rx-pg-info">Pág. {{ pagination.currentPage }}/{{ pagination.lastPage }} · {{ pagination.total }} usuarios</span>
  ...
  >
    <form id="user-form" @submit.prevent="saveUser" class="rx-form" style="display: flex; flex-direction: column; gap: 1.25rem;">
      <div class="rx-form__grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem;">
        <div class="rx-form__group rx-form__group--full">
          <label>Nombre completo</label>
          <input v-model="userForm.name" type="text" placeholder="Ej: Juan Pérez" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group">
          <label>Email</label>
          <input v-model="userForm.email" type="email" placeholder="ejemplo@correo.com" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group">
          <label>Teléfono</label>
          <input v-model="userForm.phone" type="tel" placeholder="+57 300 123 4567" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group">
          <label>Rol</label>
          <select v-model="userForm.role" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;">
            <option value="user">Usuario</option>
            <option value="support">Soporte</option>
            <option value="admin">Administrador</option>
          </select>
        ...
        <div class="rx-form__group">
          <label>Estado</label>
          <select v-model="userForm.status" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;">
            <option value="active">Activo</option>
            <option value="pending">Pendiente</option>
            <option value="inactive">Inactivo</option>
            <option value="suspended">Suspendido</option>
        ...
        <div class="rx-form__group">
          ...
          <input v-model="userForm.department" type="text" placeholder="Ej: Cauca" style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group">
          <label>Ciudad</label>
          <input v-model="userForm.city" type="text" placeholder="Ej: Popayán" style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group rx-form__group--full">
          <label>Dirección</label>
          <input v-model="userForm.address" type="text" placeholder="Calle 5 # 10-20" :required="!editingUser" style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group rx-form__group--full">
          <label>Documento de identidad</label>
          <input v-model="userForm.id_documento" type="text" placeholder="CC 1234567890" :required="!editingUser" style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
        <div class="rx-form__group rx-form__group--full">
          <label>Foto de perfil (opcional)</label>
          <input type="file" accept="image/*" @change="handlePhotoUpload" style="width: 100%; padding: 0.5rem 0; font-size: 0.85rem; color: var(--admin-text-primary);" />
          <small style="display: block; font-size: 0.75rem; color: var(--admin-text-muted); margin-top: 0.25rem;">JPEG, PNG, GIF. Máx: 10MB</small>
        </div>
        <div v-if="!editingUser" class="rx-form__group rx-form__group--full">
          <label>Contraseña</label>
          <input v-model="userForm.password" type="password" placeholder="Mínimo 8 caracteres" required style="width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-primary); font-family: inherit; font-size: 0.85rem; box-sizing: border-box;" />
        </div>
      </div>
    </form>

    <template #footer>
      <button type="button" class="rx-btn rx-btn--ghost" @click="closeUserModal" style="padding: 0.65rem 1.25rem; border-radius: 8px; font-weight: 600; font-size: 0.875rem; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-secondary); cursor: pointer;">Cancelar</button>
      <button type="submit" form="user-form" class="rx-btn rx-btn--accent" :disabled="saving" style="padding: 0.65rem 1.25rem; border-radius: 8px; font-weight: 600; font-size: 0.875rem; border: none; background: linear-gradient(135deg, #6366f1, #818cf8); color: #fff; cursor: pointer; box-shadow: 0 2px 8px rgba(99,102,241,0.25);">
  ...
  >
    <p style="margin: 0; font-size: 0.95rem; line-height: 1.5; color: var(--admin-text-secondary);">
      Esta acción eliminará permanentemente a <strong style="color: var(--admin-text-primary);">{{ userToDelete?.name }}</strong>. No se puede deshacer.
    </p>

    <template #footer>
      <button class="rx-btn rx-btn--ghost" @click="closeDeleteModal" style="padding: 0.65rem 1.25rem; border-radius: 8px; font-weight: 600; font-size: 0.875rem; border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text-secondary); cursor: pointer;">Cancelar</button>
      <button class="rx-btn rx-btn--danger" @click="deleteUser" :disabled="deleting" style="padding: 0.65rem 1.25rem; border-radius: 8px; font-weight: 600; font-size: 0.875rem; border: none; background: #ef4444; color: #fff; cursor: pointer;">
...
const toggleUserStatus = async (user: User) => {
  try { statusUpdateLoading.value[user.id] = true;
...
onUnmounted(() => { eventBus.off('user:created', handleUserCreated); eventBus.off('user:updated', handleUserUpdated); eventBus.off('user:deleted', handleUserDeleted);
...
.rx-topbar__icon { width: 48px; height: 48px; border-radius: 14px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #8b5cf6, #7c3aed); color: #fff; font-size: 1.2rem; box-shadow: 0 4px 14px rgba(139,92,246,.3); }
...
.rx-sort { font-size: .65rem; margin-left: .35rem; color: var(--txt3); opacity: .5; }
.rx-sort.active { opacity: 1; color: var(--accent-color); }
...
.rx-user-cell { display: flex; align-items: center; gap: .75rem; }
.rx-avatar-wrap { width: 38px; height: 38px; border-radius: 10px; overflow: hidden; flex-shrink: 0; }
.rx-avatar-img { width: 100%; height: 100%; object-fit: cover; }
.rx-avatar { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: .75rem; font-weight: 700; background: #ede9fe; color: #7c3aed; flex-shrink: 0; }
.rx-user-cell__info { display: flex; flex-direction: column; }
.rx-user-cell__name { font-weight: 600; color: var(--txt); font-size: .85rem; }
.rx-user-cell__sub { font-size: .72rem; color: var(--txt3); }

.rx-badge { display: inline-flex; align-items: center; gap: .3rem; padding: .25rem .65rem; border-radius: 7px; font-size: .72rem; font-weight: 600; }
.rx-date { font-size: .82rem; color: var(--txt2); }
.rx-verif { display: inline-flex; align-items: center; gap: .3rem; padding: .25rem .6rem; border-radius: 7px; font-size: .72rem; font-weight: 600; }
.rx-verif--yes { background: #ecfdf5; color: #059669; }
.rx-verif--no { background: #fef3c7; color: #d97706; }
...
.rx-act--reject:hover { background: #f59e0b; border-color: #f59e0b; }
